...
All four files live in the runtime directory (`/data` in Docker). They're loaded by every platform, but only Telegram sessions have write access (via `allowed_tools` config). Discord sessions see the same context but can't modify it.
